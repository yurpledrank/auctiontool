...
// Used as a second-opinion reference column in the AuctionPanel prob grid.
// Keys match the simulation round tokens: r32, r16, qf, sf, final, champion.

// Pool C payout structure (must mirror payout_engine.py KO_PAYOUT_PCT / N_ADVANCING)
const _KO_PCT = { r32: 0.06, r16: 0.05, qf: 0.04, sf: 0.04, final: 0.03 };
const _N_ADV  = { r32: 32,   r16: 16,   qf: 8,    sf: 4,    final: 2    };

// Compute TheAnalyst's implied Pool-C-only expected earnings at a given total pot.
// Does NOT include Pool A (bounties) or Pool B (group stage slices).
...
// True if Silver and TheAnalyst disagree by >30% relative on championship probability.
...
// USA includes +88 home-field; MEX/CAN use raw WC PELE (knockouts in USA)
...
// NELO Delta: normalizes PELE and ELO onto the same 0-1 scale (min-max of their
// respective 48-team ranges) and returns the difference as a percentage.
// Positive = Silver/PELE rates this team higher than ELO; negative = ELO rates higher.
const _PELE_VALS = Object.values(PELE_RATINGS);
const _ELO_VALS  = Object.values(ELO_RATINGS);
const _PELE_MIN  = Math.min(..._PELE_VALS), _PELE_MAX = Math.max(..._PELE_VALS);
const _ELO_MIN   = Math.min(..._ELO_VALS),  _ELO_MAX  = Math.max(..._ELO_VALS);

export function neloDelta(team) {
  const pele = PELE_RATINGS[team], elo = ELO_RATINGS[team];
  if (!pele || !elo) return null;
  const pNorm = (pele - _PELE_MIN) / (_PELE_MAX - _PELE_MIN);
  const eNorm = (elo  - _ELO_MIN)  / (_ELO_MAX  - _ELO_MIN);
  return (pNorm - eNorm) * 100;
}
